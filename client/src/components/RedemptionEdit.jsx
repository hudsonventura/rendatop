...
export default function RedemptionEdit({ redemption, setReload, externalOpen, onExternalClose }) {
    ...
    useEffect(() => {
        if (!redemption || !isOpen) return
        form.reset({
            title: redemption.title ?? "",
            date: redemption.date ? new Date(redemption.date) : new Date(),
            value: redemption.value
                ?.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
                ?? "",
        })
    }, [form, redemption, isOpen])
    ...
    const onSubmit = (values) => {
        ...
        axiosInstance
            .patch(`/Redemptions/${redemption.id}`, JSON.stringify(payload))
            .then(() => {
                setIsOpen(false)
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="max-w-xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Ajuste os dados do resgate selecionado.
                    </DialogDescription>
                </DialogHeader>
